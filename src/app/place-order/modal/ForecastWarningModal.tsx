                ...
                <div key={idx} className="border border-red-200 rounded-lg p-3 bg-red-50">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-red-600 font-black">
                      Thiếu {forecast.shortage.toLocaleString('en-US')} {forecast.packageProductId ? 'kiện' : 'sản phẩm'}
                    </span>
                  </div>
                  <div className="text-xs text-gray-600 mt-1">
                    <div>Yêu cầu: {forecast.requiredQuantity.toLocaleString('en-US')}</div>
                    <div>Dự tính có: {forecast.estimatedQuantity.toLocaleString('en-US')}</div>
                    <div>Hiện tại: {forecast.currentQuantity.toLocaleString('en-US')}</div>
                  </div>
                </div>
              ))}
          </div>
